...
export const StatsSection: React.FC<StatsSectionProps> = ({
  ...
  const fetchStats = async () => {
    try {
      ...
      const categories = [
        ...new Set(results.map((r) => r.category?.toString())),
      ].filter(Boolean);
        ...
        >
          ...
          {connectionError && (
            ...
            >
              ...
              <p className="text-sm">
                يتم عرض بيانات تجريبية حالياً - البيانات الفعلية ستظهر عند توفر
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-6xl mx-auto">
            {Object.entries(stats.categoriesCount)
              .filter(
                ([category]) => category !== "2" && category !== "غير محدد"
              )
              .map(([category, count]) => (
                <div
                  key={category}
                  className={`p-4 rounded-xl text-center hover:shadow-lg transition-all duration-300 hover:scale-105 ${
                    isDarkMode
            ...
            {Object.keys(stats.categoriesCount).filter(
              (category) => category !== "2" && category !== "غير محدد"
            ).length === 0 && (
